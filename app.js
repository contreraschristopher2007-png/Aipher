...
function loadState() {
  try {
    ...
    // Migración: eliminar variables muertas de versiones anteriores
...
document.addEventListener('DOMContentLoaded', () => {
  // Resetear estado de voz al cargar (por si se recargó durante una sesión)
  voiceSession = false;
  voiceBusy = false;
  speaking = false;
  speakQueue = [];
...
function setEngine(engine) {
  ...
  // 🚨 CRÍTICO: Limpiar interval anterior para evitar acumulación
...
function speak(text, done) {
  ...
  const chunks = cleanText.match(/.{1,CONFIG.maxSpeechChars}/g) || [cleanText];
